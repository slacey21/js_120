...
const HIGHEST_FEATURED = 9876543201;

function featured(number) {
  let nextOddSevenMultiple = toOddSevenMultiple(number);

  while (nextOddSevenMultiple <= HIGHEST_FEATURED) {
    if (allUnique(nextOddSevenMultiple)) return nextOddSevenMultiple;
    nextOddSevenMultiple += 14;
  }
...
function allUnique(number) {
  ...
  numArr.forEach(number => {
    if (!uniqueDigits.includes(number)) uniqueDigits.push(number);
  });

  return numArr.length === uniqueDigits.length;
}

function toOddSevenMultiple(number) {
  do {
    number += 1;
